...
export default class TipoAfastamentoPostgresRepository implements TipoAfastamentoRepository {
  ...
  async getAll(): Promise<TipoAfastamentoEntity[]> {
    try {
      const tiposAfastamento = await conn.query(
        "SELECT ID, TIPO_AFASTAMENTO FROM TIPOS_AFASTAMENTO",
      );
  ...
  async getById(id: number): Promise<TipoAfastamentoEntity> {
    try {
      const tiposAfastamento = await conn.query(
        `SELECT ID, TIPO_AFASTAMENTO FROM TIPOS_AFASTAMENTO WHERE ID = ${id}`,
      );
      return tiposAfastamento.rows[0];
    } catch (error) {
      throw new AppError(error.message, status.INTERNAL_SERVER);
    }
  }
  ...
  async getByIdExisting(id: number): Promise<number> {
    try {
      const tiposAfastamento = await conn.query(
        `SELECT ID FROM TIPOS_AFASTAMENTO WHERE ID = ${id}`,
      );
